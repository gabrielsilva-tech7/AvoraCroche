    ...
    <article class="product-card">
      ...
      <div class="product-info">
        ...
        <div class="product-bottom">
          ...
          <button class="add-btn" onclick="addToCart(${product.id})" aria-label="Adicionar ${product.name} ao carrinho">+</button>
        </div>
      </div>
    </article>
  `).join("");
...
function renderCart() {
  ...
  if (!cart.length) {
    cartItems.innerHTML = `<div class="cart-empty"><p>Seu carrinho está vazio.</p><small>Escolha uma peça da coleção para começar.</small></div>`;
  } else {
    cartItems.innerHTML = cart.map((item, index) => `
      <div class="cart-item">
        <img src="${item.image}" alt="${item.name}" />
        <div><h4>${item.name}</h4><p>${money.format(item.price)}</p></div>
        <button class="remove-item" onclick="removeFromCart(${index})" aria-label="Remover ${item.name}">×</button>
      </div>
    `).join("");
  }
...
function openWhatsApp(message) {
  if (!WHATSAPP_NUMBER) {
    showToast("Adicione o número do WhatsApp no script.js antes de publicar");
    return;
  }
...
document.getElementById("checkoutButton").addEventListener("click", () => {
  ...
  const lines = cart.map(item => `• ${item.name} — ${money.format(item.price)}`);
  const total = cart.reduce((sum, item) => sum + item.price, 0);
  openWhatsApp(`Olá! Quero fazer um pedido na Ávora:\n\n${lines.join("\n")}\n\nTotal: ${money.format(total)}\n\nPodemos combinar os detalhes?`);
});

document.getElementById("whatsappContact").addEventListener("click", () => {
  openWhatsApp("Olá! Vim pelo site da Ávora e gostaria de fazer uma encomenda.");
});

document.getElementById("instagramLink").href = INSTAGRAM_URL;
document.getElementById("instagramLink").target = "_blank";
document.getElementById("instagramLink").rel = "noopener noreferrer";
...
if (installButton) {
  installButton.addEventListener("click", async () => {
    if (!deferredInstallPrompt) {
      showToast("No iPhone, use Compartilhar → Adicionar à Tela de Início. No Android, abra o menu do navegador → Instalar app.");
      return;
    }
...
window.addEventListener("appinstalled", () => {
  ...
  showToast("Ávora instalada no seu celular");
});
